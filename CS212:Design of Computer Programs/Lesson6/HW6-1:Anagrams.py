...
def anagrams(phrase, shortest=2):
    ...
    # your code here
    res = set()
    def find_anagrams(letters, anagram, prev_word, shortest):
        #print 'a:',anagram, '  l:',letters
        if letters == '': res.add(anagram[1:])
        for w in sorted(find_words(letters)):
            if len(w) >= shortest and w >= prev_word:
                prev_word = w
                find_anagrams(removed(letters, w), anagram+ ' ' + w, w, shortest)
        return res
