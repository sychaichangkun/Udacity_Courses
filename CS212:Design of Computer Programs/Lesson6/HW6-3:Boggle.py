...
def boggle_words(board, minlength=3):
    ...
    N = size(board)
    res = set()
    def find_bwords(cur, prefix, board, minlength):
        if prefix in WORDS and len(prefix) >= minlength: res.add(prefix)
        if prefix not in PREFIXES: return
        for i in neighbors(cur, N):
            if board[i] != '|':
                l = board[i]
                board = board[:i]+'|'+board[i+1:]
                find_bwords(i, prefix+l, board, minlength)
                board = board[:i]+l+board[i+1:]
    for i in range(N*N):
        if board[i] != '|': find_bwords(i,'',board,minlength)
    return res
