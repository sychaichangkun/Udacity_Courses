...
def grid(cars, N=N):
    ...
    goal = (('@', (N * ((N + 1) / 2) - 1,)),)
    walls_index = tuple([i for i in xrange(N * N) if (i < N or i >= N * (N - 1)
                                                      or i % N == 0 or (i + 1) % N == 0) if i != N * ((N + 1) / 2) - 1])
    walls = (('|', walls_index),)
...
def successors(state):
    ...
    for car, locations in state:
        if car != '@' and car != '|':
            loc_unreachable = get_unreachable(car, state)
            new_res = possible_state_action(
                car, locations, state, loc_unreachable)
            res = dict(res.items() + new_res.items())
    return res


def get_unreachable(car, state):
    res = ()
    for s, loc in state:
        if s != '@' and s != car:
            res = res + loc
...
def possible_state_action(car, locations, state, loc_unreachable):
    res = {}
    dir = True  # left-right
    if abs(locations[0] - locations[1]) == N:
        dir = False  # up-down
    if dir:
        for i in range(1, N - 2):
            if reachable(i, locations, loc_unreachable):
                new_state = get_new_state(state, i, locations, car)
                res[tuple(sorted(new_state))] = (car, i)
            else:
                break
...
def get_new_state(state,  i, locations, car):
    new_state = ()
    for x, y in state:
        if x != car:
            new_state += ((x, y),)
    new_state = new_state + \
        ((car, tuple([loc + i for loc in locations])),)
    return new_state


def reachable(move, locations, loc_unreachable):
    for i in locations:
        if i + move in loc_unreachable:
            return False
    return True
