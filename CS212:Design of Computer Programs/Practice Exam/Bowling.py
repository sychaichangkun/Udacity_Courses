...
def bowling(balls):
    ...
    for frame in xrange(1,11):
        score, j =  get_frame_score(balls, i)
        total_score += score
        i += j
    return total_score
    
def get_frame_score(balls, i):
    if balls[i] == 10: 
        return balls[i] + balls[i+1] + balls[i+2],1
    if balls[i] + balls[i+1] == 10: 
        return balls[i] + balls[i+1] + balls[i+2],2
    else:
        return balls[i] + balls[i+1],2
