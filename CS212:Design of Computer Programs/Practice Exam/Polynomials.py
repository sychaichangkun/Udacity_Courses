...
def poly(coefs):
    ...
    # your code here (I won't repeat "your code here"; there's one for each
    # function)
    poly.__name__ = ''
    poly.coefs = coefs
    for ith, coef in enumerate(coefs):
        if ith == 0:
            itemname = str(coef)
        elif ith == 1:
            itemname = 'x'
        ...
        if coef == 0:
            ...
            itemname = str(coef) + ' * ' + itemname

        if ith == 0 or poly.__name__ == '':
            poly.__name__ = itemname
        elif itemname != '':
            poly.__name__ = itemname + ' + ' + poly.__name__
...
def add(p1, p2):
    ...
    for i,coef in enumerate(p1.coefs):
        coefs[i] = coef
    for i,coef in enumerate(p2.coefs):
        coefs[i] += coef
...
def mul(p1, p2):
    ...
    coefs = [0] * (len(p1.coefs) + len(p2.coefs) -1)
    for i,icoef in enumerate(p1.coefs):
        for j,jcoef in enumerate(p2.coefs):
            coefs[i+j] += icoef * jcoef
    return poly(tuple(coefs))
...
def deriv(p):
    ...
    newcoef = []
    for i,coef in enumerate(p.coefs[1:],1):
        newcoef = newcoef + [i * coef]
    return poly(tuple(newcoef)) if len(p.coefs) > 0 else poly((0,))

def integral(p, C=0):
    "Return the integral of a function p (with respect to its argument)."
    newcoef = [C]
    for i,coef in enumerate(p.coefs,1):
        newcoef = newcoef + [1.0/i * coef]
    return poly(tuple(newcoef)) if len(p.coefs) > 0 else poly((0,))
...
class poly():
    def __init__(self,args):
        ...
        self.__name__ = ''
        for ith, coef in enumerate(self.coefs):
            if ith == 0:
                itemname = str(coef)
            elif ith == 1:
                itemname = 'x'
            ...
            if ith == 0 or self.__name__ == '':
                self.__name__ = itemname
            elif itemname != '':
                self.__name__ = itemname + ' + ' + self.__name__
    ...
    def __add__(self, arg):
        if isinstance(arg, int):
            arg = poly(arg)
        coefs = [0] * max(len(arg.coefs),len(self.coefs))
        for i,coef in enumerate(arg.coefs):
            coefs[i] = coef
    ...
    def __sub__(self,arg):
        if isinstance(arg, int):
            arg = poly(arg)
        coefs = [0] * max(len(self.coefs),len(arg.coefs))
        for i,coef in enumerate(self.coefs):
            coefs[i] = coef
        for i,coef in enumerate(arg.coefs):
            coefs[i] -= coef
    ...
    def __mul__(self,arg):
        if isinstance(arg, int):
            arg = poly(arg)
        coefs = [0] * (len(self.coefs) + len(arg.coefs) -1)
        for i,icoef in enumerate(self.coefs):
            for j,jcoef in enumerate(arg.coefs):
                coefs[i+j] += icoef * jcoef
        return poly(tuple(coefs))   
