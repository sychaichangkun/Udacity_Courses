...
def compile_formula(formula, verbose=False):
    ...
    parms = ', '.join(letters)
    splitword = re.split('([A-Z]+)', formula)
    tokens = map(compile_word, splitword)
    #print 'tokens:', tokens
    #print 're.split():', re.split('([A-Z]+)', formula)
    headletters = '['+','.join([word[0] for word in splitword if re.search('[A-Z]',word)])+']'
    #print 'headletters:', headletters
    #body = ''.join(tokens)
    f = 'lambda %s: %s if %s.count(0)==0 else False' % (parms, ''.join(tokens), headletters)
    if verbose: print f
