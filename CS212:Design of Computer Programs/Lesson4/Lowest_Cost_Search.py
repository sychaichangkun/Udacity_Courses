...
def lowest_cost_search(start, successors, is_goal, action_cost):
    ...
    def lowest_cost(p):
        return action_cost(p[-2])
    
    while frontier:
        path = frontier.pop(min(frontier,key = lowest_cost))
        if is_goal(path[-1]):
            return path
        for action, state in successors(path[-1]):
            if state not in explored.keys() or action_cost(action) < explored[state]:
                explored[state] = action_cost(action)
