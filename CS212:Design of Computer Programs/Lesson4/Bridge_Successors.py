...
def bsuccessors(state):
    ...
    if 'light' in here:
        ...
        there = there | frozenset(['light'])
        for i in here:
            for j in here:
                    here = here - frozenset([i,j])
                    there = there | frozenset([i,j])
                    res[(here - frozenset([i,j]), there | frozenset([i,j]), t + max(int(i),int(j)))] = (i,j,'->')  
        return res
    else:
        here = here | frozenset(['light'])
        there = there - frozenset(['light'])
        for i in there:
            res[(here | frozenset([i]), there - frozenset([i]), t + int(i))] = (i, i, '<-')
        return res
    # your code here  
