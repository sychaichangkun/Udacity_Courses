...
def subway(**lines):
    ...
    for l, s in lines.items():
        stations = s.split(' ')
        stationnum = len(stations)
        for i in stations:
            if not sub.has_key(i):
                sub[i] = []
        for i in range(stationnum):
            if i == 0 and i < stationnum - 1:
                sub[stations[i]] = sub[stations[i]] + [(stations[i+1],l)]
            elif i == stationnum - 1 and i > 0:
                sub[stations[i]] = sub[stations[i]] + [(stations[i-1],l)]
            else:
                sub[stations[i]] = sub[stations[i]] + [(stations[i-1],l),(stations[i+1],l)]
...
def ride(here, there, system=boston):
    ...
    def isgoal(s):
        return s == there
    return shortest_path_search(here, succ, isgoal)
    
def longest_ride(system):
    """"Return the longest possible 'shortest path' 
    ride between any two stops in the system."""
    ## your code here
    return max([ride(i,j,system) for i in system.keys() for j in system.keys()],key=len)
