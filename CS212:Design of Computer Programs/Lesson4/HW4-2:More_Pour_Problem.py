...
def more_pour_problem(capacities, goal, start=None):
    ...
    def successors(s):
        ...
        for i in range(nums):
            state = s[:]
            state[i] = 0
            res[tuple(state)] = ('empty', i)
            state[i] = capacities[i]
            res[tuple(state)] = ('fill', i)
            for j in range(nums):#pour i into j
                state = s[:]
                if state[i] + state[j] > capacities[j]:
                    state[i] = state[i] + state[j] - capacities[j] 
                    state[j] = capacities[j]
                else:
                    state[j] = state[i] +state[j]
                    state[i] = 0
                res[tuple(state)] = ('pour', i ,j)
#        print res
        return res
